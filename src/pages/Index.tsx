...
import { products as dummyProducts, testimonials } from '@/data/products';
...
const Index = () => {
  ...
  const featuredProducts = combinedProducts
    .filter(p => p.badge === 'bestseller' || p.badge === 'sale')
    .slice(0, 4);
  const newArrivals = combinedProducts.filter(p => p.badge === 'new');
